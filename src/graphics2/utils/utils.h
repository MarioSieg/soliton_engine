...
#ifdef APP_DEBUG
	#define CHECK(x) { if(!(x)) { LOG_FATAL("Check error, at line {0} on file {1}.", __LINE__, __FILE__); __builtin_trap(); } }
	#define ASSERT(x, ...) { if(!(x)) { LOG_FATAL("Assert failed: {2}, at line {0} on file {1}.", __LINE__, __FILE__, __VA_ARGS__); __builtin_trap(); } }
#else
...
namespace engine
{
	...
	static inline unsigned char srgbToLinear(unsigned char inSrgb)
	{
		...
		return static_cast<unsigned char>(lin * 255.0f);
	}

	static inline unsigned char linearToSrgb(unsigned char inlin)
	{
		float lin = inlin / 255.0f;
		if (lin < 0.00313067f) return static_cast<unsigned char>(lin * 12.92f * 255.0f);
		float srgb = math::pow(lin, (1.0f / 2.4f)) * 1.055f - 0.055f;

		return static_cast<unsigned char>(srgb * 255.0f);
	}
		...
		const std::filesystem::path& longPath)
	{
		...
		//if (string_s("\\") || result.starts_with("/"))
		//{
	...
	inline static std::filesystem::path buildStillNonExistPath(const std::filesystem::path& p)
	{
		...
		{
			pUnique = rawPath + utf8::utf8to16(fmt::format("_{}", num));
			num ++;
		}
