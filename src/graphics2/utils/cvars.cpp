	...
	void CVarSystem::exportAllConfig(const std::string& path)
	{
		...
		for (const auto& cVarCategoryPair : cVarCategories)
		{
			...
			for (const auto* cVar : cVarCategoryPair.second)
			{
				...
				{
					cVarSection[cVar->name] = fmt::format("{} # {}", *getCVar<int32_t>(cVar->name), cVar->description);
				}
				else if (cVar->type == CVarType::Bool)
				{
					cVarSection[cVar->name] = fmt::format("{} # {}", *getCVar<bool>(cVar->name), cVar->description);
				}
				else if (cVar->type == CVarType::String)
				{
					cVarSection[cVar->name] = fmt::format("{} # {}", *getCVar<std::string>(cVar->name), cVar->description);
				}
				else if (cVar->type == CVarType::Float)
				{
					cVarSection[cVar->name] = fmt::format("{} # {}", *getCVar<float>(cVar->name), cVar->description);
				}
				else
				{
	...
	bool CVarSystem::importConfig(const std::string& path)
	{
		...
		for (const auto& section : ini.sections)
		{
			...
			for (const auto& cVarPair : keymap)
			{
				...
				{
					if (valueStr == "false" || valueStr == "FALSE" || valueStr == "0")
					{
