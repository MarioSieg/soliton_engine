...
namespace lu::graphics {
    ...
    public:
        ...
        [[nodiscard]] inline auto get_path() const noexcept -> const eastl::string& { return m_path; }
        [[nodiscard]] inline auto get_stage() const noexcept -> shader_stage { return m_stage; }
        [[nodiscard]] inline auto get_keys() const noexcept -> const ankerl::unordered_dense::map<eastl::string, std::int32_t>& { return m_macro_values; }
        [[nodiscard]] inline auto get_macros() const noexcept -> const ankerl::unordered_dense::set<eastl::string>& { return m_macros; }
        ...
        [[nodiscard]] inline auto get_entry_point() const noexcept -> const eastl::string& { return m_entry_point; }
        inline auto set_entry_point(const eastl::string& entry_point) noexcept -> void { m_entry_point = entry_point; }
        inline auto set_int(const eastl::string& key, const std::int32_t value) noexcept -> void { m_macro_values[key] = value; }
        inline auto set_bool(const eastl::string& key, const bool value) noexcept -> void { m_macro_values[key] = value; }
        inline auto set_macro(const eastl::string& macro) noexcept -> void { m_macros.insert(macro); }
