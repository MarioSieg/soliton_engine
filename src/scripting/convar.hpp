    ...
    public:
        ...
        [[nodiscard]] auto operator ()() -> T {
            if (m_is_locked) [[unlikely]] {
                log_error("Failed to get CONVAR '{}': Variable write is locked", full_name().c_str());
                return fallback();
            }
            register_var();
            if (!m_ref) [[unlikely]] {
                log_error("Failed to get CONVAR '{}': Engine config key not valid", full_name().c_str());
                return fallback();
            }
            ++m_gets;
            T rv {};
            if constexpr (std::is_same_v<T, eastl::string>) {
                rv = m_ref->cast<std::string>().valueOr(fallback().c_str()).c_str();
            } else {
                rv = m_ref->cast<T>().valueOr(fallback());
            }
        ...
        [[nodiscard]] auto full_name() const -> const eastl::string& { return m_name; }

    private:
        auto register_var() {
            if (m_ref) return;
            passert(detail::tid == std::this_thread::get_id() && "Convars must be registered on the main thread");
            eastl::string s_fallback {"None"};
            if constexpr (std::is_same_v<T, eastl::string> | std::is_same_v<T, bool>) {
                if (m_fallback) {
            ...
            if constexpr (!std::is_same_v<T, bool> && std::is_integral_v<T> || std::is_floating_point_v<T>) {
                if (m_min > m_max) [[unlikely]] {
                    panic("Failed to create CONVAR '{}': The min value is greater than the max value", full_name().c_str());
                    return;
                }
                if constexpr (std::is_floating_point_v<T>) {
                ...
                if (!m_min.empty() || !m_max.empty()) [[unlikely]] {
                    panic("Failed to create CONVAR '{}', string/bool variables cannot have min/max clamps", full_name().c_str());
                    return;
                }
            }
            ...
            if (!root || !root->isTable()) [[unlikely]] {
                log_error("Failed to register CONVAR '{}': Engine config root table is not initialized yet", full_name().c_str());
                return;
            }
            refs.emplace(*root);
            std::stringstream ss {m_name.c_str()};
            for (std::string k {}; std::getline(ss, k, '.');) {
                if (refs.top().isTable()) {
                    refs.emplace(refs.top()[k]);
                } else {
                    log_error("Failed to register CONVAR '{}': Engine config root table is not a table", full_name().c_str());
                    return;
                }
            }
            auto key = refs.top();
            if (!key.isValid()) [[unlikely]] {
                log_error("Failed to register CONVAR '{}': Engine config key value is not valid", full_name().c_str());
                return;
            }
            if constexpr (!std::is_same_v<T, bool> && (std::is_integral_v<T> || std::is_floating_point_v<T>)) {
                if (!key.isNumber()) {
                    log_error("Failed to register CONVAR '{}': Engine config key value is not of type 'number': '{}'", full_name().c_str(), key.tostring());
                    return;
                }
            } else if constexpr (std::is_same_v<T, bool>) {
                if (!key.isBool()) {
                    log_error("Failed to register CONVAR '{}': Engine config key value is not of type 'boolean': '{}'", full_name().c_str(), key.tostring());
                    return;
                }
            } else if constexpr (std::is_same_v<T, eastl::string>) {
                if (!key.isString()) {
                    log_error("Failed to register CONVAR '{}': Engine config key value is not of type 'string': '{}'", full_name().c_str(), key.tostring());
                    return;
                }
            }
            m_ref = key;
