...
vec3 lensFlare(vec2 texcoord, vec2 sunCoord, out vec3 ghostColor)
{
    ...
    float fov = uboPerFrame.cameraInfo.x;
    float aspect = uboPerFrame.cameraInfo.y;
    coord.x *= aspect;
    sunPos.x *= aspect;
    ...
    const float invTanHalfFovy = 1.0f / tan(fov * 0.5f);
    ...
    ghostColor = uboPerFrame.sunColor.rgb * vec3(gl * GLARE_BRIGHTNESS);
    vec3 lf = uboPerFrame.sunColor.rgb * fl * FLARE_BRIGHTNESS;
